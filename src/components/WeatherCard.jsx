...
import { Droplets, Wind, Sun, Clock, CalendarDays, Activity, Eye, Thermometer, Sunrise, Sunset } from 'lucide-react';
    ...
    <div className="bento-grid animate-fade-in">
      ...
      <div className="bento-card main-weather-tile">
        <div className="main-info">
          ...
          <h1 className="main-temp">{formatTemp(current.temp)}</h1>
      ...
      {/* 8. FEELS LIKE METRIC */}
      <div className="bento-card metric-tile">
        <h3 className="section-title"><Thermometer size={16} /> Feels Like</h3>
        <div className="metric-value">{formatTemp(current.feelsLike)}</div>
        <div className="metric-sub">
          {current.feelsLike > current.temp ? 'Warmer than actual.' : current.feelsLike < current.temp ? 'Colder than actual.' : 'Similar to actual.'}
        </div>
      </div>

      {/* 9. VISIBILITY METRIC */}
      <div className="bento-card metric-tile">
        <h3 className="section-title"><Eye size={16} /> Visibility</h3>
        <div className="metric-value">{(current.visibility / 1000).toFixed(1)}</div>
        <div className="metric-sub">kilometers</div>
      </div>

      {/* 10. SUNRISE/SUNSET METRIC */}
      <div className="bento-card metric-tile">
        <h3 className="section-title"><Sunrise size={16} /> Sun</h3>
        <div className="metric-value small-text-value">
          <div className="sun-time"><Sunrise size={18} className="sun-icon-inline" /> {current.sunrise}</div>
          <div className="sun-time"><Sunset size={18} className="sun-icon-inline" /> {current.sunset}</div>
        </div>
      </div>
